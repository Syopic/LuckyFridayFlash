package ua.com.syo.luckyfriday.view
{
	...
	public class TerrainView extends Sprite {
		...
		public function TerrainView(space:Space):void {

			var w:uint = 1024;
			var h:uint = 600;
			// Initialise terrain bitmap.
			var bitmap:Bitmap = new CaveC();
			var bit:BitmapData = bitmap.bitmapData
	...
	public function invalidate(region:AABB, space:Space):void {
		...
		for (var y:int = y0; y <= y1; y++) {
			for (var x:int = x0; x <= x1; x++) {
				...
				// Recycle list nodes
				polys.clear();
				b.setShapeMaterials(new Material(0.8,1.0,1.4,1.5,0.01));
				b.space = space;
			}}
	}
